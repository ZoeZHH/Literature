...
Plot raw data for TS_5 vs. FCO2 & TS_5 vs. FCH4.

```{r}
plot_flux_by_month <- function(data, x_var, y_var, y_label) {
  ggplot(data, aes_string(x = x_var, y = y_var, color = "factor(month)")) +
    geom_point(size = 3, shape = 19) + 
    labs(x = x_var, y = y_label, title = paste("Scatterplot of", y_label, "by Month")) +
...
# Loop over each vegetation type to fit model and plot
for (ss in unique(df$study_site)) {
  # Subset the data for the vegetation type
  subset_data <- subset(df, study_site == ss)
  # Remove rows with non-finite peat temperatures or FCH4 fluxes
  ...
  if (length(subset_data$peat_temp_5cm) > 1) {
    ...
    model <- nls(FCH4_EXP ~ a * exp(b * peat_temp_5cm), data = subset_data,
    ...
    # Predict FCH4 values using the fitted model
...
# Display the plots (you can also arrange them in a grid if you like)
if (!is.null(plot_list[['PSLS']])) {
  print(plot_list[['PSLS']])
}
if (!is.null(plot_list[['PSTS']])) {
  print(plot_list[['PSTS']])
}
if (!is.null(plot_list[['PSW']])) {
  print(plot_list[['PSW']])
}
```
...
plot_list <- list()
# Loop over each vegetation type to fit model and plot
for (veg_type in unique(df$vegetation_type)) {
  # Subset the data for the vegetation type
  subset_data <- subset(df, vegetation_type == veg_type)
  # Remove rows with non-finite peat temperatures or FCH4 fluxes
  ...
  if (length(subset_data$peat_temp_5cm) > 1) {
    ...
    p <- ggplot(subset_data, aes(x = peat_temp_5cm, y = FCO2_EXP)) +
      ...
      ggtitle(paste('FCO2-Q10 Curve for', veg_type, '(Q10 =', sprintf('%.2f', Q10), ')')) +
      xlab('Peat Temperature at 5cm (°C)') +
      ylab('FCO2 Flux (µmol m⁻² s⁻¹)') +
      theme_minimal()

    # Store the plot in the list
    plot_list[[veg_type]] <- p
  } else {
    warning(paste('Not enough data points to fit a curve for', veg_type))
  }
}
